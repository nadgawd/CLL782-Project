...
\begin{frame}{3.2 Problem Statement}
    ...
    \begin{itemize}
        \item \textbf{Dual-compartment bins} (Wet + Dry) per SWM Rules 2016.
            \begin{itemize}
                \item \textcolor{HighlightGreen}{Green}: Compostable / Organic (food waste).
                \item \textcolor{AccentBlue}{Blue}: Recyclable (paper, plastic, bottles).
            \end{itemize}
        \item Each unit has a \textbf{cost} $C$, \textbf{service radius} $R$, and \textbf{capacity} $K$.
...
\begin{frame}{3.2 Variables, Constants \& Data}
    \begin{table}
        ...
        \begin{tabular}{l p{4.8cm} l l}
            ...
            $y_{j}$ & Install dual-bin at location $j$? & $\{0,1\}$ & \textbf{Binary Decision} \\
            $a_{i,j}$ & Fraction of zone $i$ demand $\to$ bin $j$ & $[0,1]$ & Continuous Decision \\
            ...
            $R$ & Service radius & 40 m & Constant \\
            $K$ & Bin capacity (dual FRP unit) & 30 kg & Constant \\
            $C$ & Bin procurement cost (per unit) & Rs.\,12K & Constant \\
            $B$ & Total budget & Rs.\ & Constraint \\
            \bottomrule
        \end{tabular}
    \end{table}

    \footnotesize \textbf{Ref:} SWM Rules 2016 (Wet+Dry segregation); Disney 30-ft rule; Glasdon norms (30--50 m).
\end{frame}

% --- Slide 3/4 ---
\begin{frame}{3.2 Mathematical Formulation (MILP)}
    \begin{exampleblock}{Minimize Total User Inconvenience}
        $$ \text{Min } Z = \sum_{i=1}^{m} \sum_{j=1}^{p} F_i \cdot a_{i,j} \cdot D_{ij} $$
    \end{exampleblock}

    \textbf{Subject to:}
    \begin{enumerate}
        \item \textbf{Coverage}: Every zone must be fully served.
            $\displaystyle\sum_{j} a_{i,j} = 1 \quad \forall\, i$
        ...
        \item \textbf{Capacity}: Bins must not overflow.
            $\displaystyle\sum_{i} F_i \cdot w \cdot a_{i,j} \le K \cdot y_{j} \quad \forall\, j$
        \item \textbf{Accessibility}: $a_{i,j} = 0$ if $D_{ij} > R$
        \item \textbf{Budget}: $\displaystyle\sum_{j} C \cdot y_{j} \le B$
    \end{enumerate}
\end{frame}
...
\begin{frame}{3.2 Optimization Analysis \& Insights}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Complexity}
                \begin{itemize}
                    ...
                    \item Discretize ROI into 137 cells ($50\text{m} \times 50\text{m}$).
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Expected Outcomes}
                \begin{itemize}
                    ...
                    \item \textbf{Sparse} along Main Road (low $F_i$, $R$ constraint).
    ...
    \begin{itemize}
        ...
        \item At $K = 30$ kg/bin $\Rightarrow$ minimum $\sim$200 dual-bins needed.
...
\begin{frame}{Integrated Blueprint \& Next Steps}
    \begin{block}{System-Level Summary}
        \begin{enumerate}
            ...
            \item \textbf{Mod 3.2}: Place $\sim$200 dual-bins (Wet+Dry) within 40 m radius to capture waste at source.
